...
func main() {
	...
	for {
		...
		ins_str:="insert into book_borrow_history (stu_id,book_name,start_time,end_time) values(\""+arr[1]+"\",\""+arr[5]+"\","+start_time+","+end_time+")";
		fmt.Printf("%d: %s\n",success_insert+1, ins_str)
		waitgroup.Add(1)
		wch <- struct{}{}
		go func (ins_str string){
			for SqlInsert(ins_str) !=nil{
				fmt.Printf("重新执行 %s\n",ins_str)
			}
			<- wch
			waitgroup.Done()
